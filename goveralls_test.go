...
func TestVerboseArg(t *testing.T) {
	...
	t.Run("with verbose", func(t *testing.T) {
		cmd := exec.Command("goveralls", "-package=github.com/yuin/goveralls/tester", "-v", "-endpoint")
	...
	t.Run("without verbose", func(t *testing.T) {
		cmd := exec.Command("goveralls", "-package=github.com/yuin/goveralls/tester", "-endpoint")
		cmd.Args = append(cmd.Args, "-v", "-endpoint", fs.URL)
		b, err := cmd.CombinedOutput()
		if err != nil {
			t.Fatal("Expected exit code 0 got 1", err, string(b))
...
func TestShowArg(t *testing.T) {
	...
	t.Run("with show", func(t *testing.T) {
		cmd := exec.Command("goveralls", "-package=github.com/yuin/goveralls/tester/...", "-show", "-endpoint")
		...
		expected := `goveralls: github.com/yuin/goveralls/tester
Fake message
http://fake.url
`
...
func TestRaceArg(t *testing.T) {
	...
	t.Run("it should pass the test", func(t *testing.T) {
		cmd := exec.Command("goveralls", "-package=github.com/yuin/goveralls/tester", "-race")
...
func TestGoveralls(t *testing.T) {
	...
	runCmd(t, "go", "get", "github.com/yuin/goveralls/testergo-runewidth")
	b := runCmd(t, "goveralls", "-package=github.com/yuin/goveralls/tester")
...
func prepareTest(t *testing.T) (tmpPath string) {
	...
	runCmd(t, "go", "build", "-o", filepath.Join(tmp, "bin", "goveralls"), "github.com/yuin/goveralls")
